...
function handleErrorsAndMore() {
	...
	process.on('unhandledRejection', reason => {
		if (reason && reason.stack) {
			...
			log.error('caught:unhandledRejection', reason);
		}
	});
...
function prettifyLog(jsonLog) {
	...
	const header = (logLevelColor[logLevelStr] || logLevelColor.def)(time + logLevelStr.toUpperCase());
	const stringRep = util.inspect(jsonLog, false, null, chalk.supportsColor);
